...
//获取患病时间
const getIllnessTime = (time: IllnessTime) => timeOptions.find((item) => item.value === time)?.label

//获取是否就诊
const getConsultFlag = (flag: 0 | 1) => flagOptions.find((item) => item.value === flag)?.label
...
<template>
  ...
  <div class="msg msg-illness" v-if="item.msgType === MsgType.CardPat">
    <div class="patient van-hairline--bottom">
      ...
      <p v-if="item.msg.consultRecord?.illnessTime">
